...
<div class="space-y-12">
    ...
    <section class="space-y-6 text-center">
        @guest
            ...
            <div class="flex flex-wrap items-center justify-center gap-4 pt-4">
                <a href="{{ localized_route('register') }}" 
                   class="inline-flex items-center justify-center rounded-full bg-indigo-600 px-6 py-3 text-base font-semibold text-white shadow-lg shadow-indigo-500/30 transition hover:bg-indigo-500">
                    {{ __('home.get_started') }}
                </a>
                <a href="{{ localized_route('jobs.index') }}" 
                   class="inline-flex items-center justify-center rounded-full border-2 border-indigo-600 px-6 py-3 text-base font-semibold text-indigo-600 transition hover:bg-indigo-50 dark:border-indigo-400 dark:text-indigo-400 dark:hover:bg-indigo-500/10">
                    {{ __('home.browse_jobs') }}
                </a>
            </div>
        @else
        ...
        <div class="mx-auto grid max-w-4xl grid-cols-2 gap-4 pt-8 sm:grid-cols-4">
            <div class="rounded-xl border border-slate-200/60 bg-white/60 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40">
                <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                    {{ $metrics['total_roles'] ?? 0 }}
                </div>
                <div class="text-sm text-slate-600 dark:text-slate-400">{{ __('home.active_roles') }}</div>
            </div>
            <div class="rounded-xl border border-slate-200/60 bg-white/60 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40">
                <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                    {{ $metrics['remote_roles'] ?? 0 }}
                </div>
                <div class="text-sm text-slate-600 dark:text-slate-400">{{ __('home.remote_jobs') }}</div>
            </div>
            <div class="rounded-xl border border-slate-200/60 bg-white/60 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40">
                <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                    {{ $metrics['new_this_week'] ?? 0 }}
                </div>
                <div class="text-sm text-slate-600 dark:text-slate-400">{{ __('home.new_this_week') }}</div>
            </div>
            <div class="rounded-xl border border-slate-200/60 bg-white/60 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40">
                <div class="text-3xl font-bold text-indigo-600 dark:text-indigo-400">
                    {{ $metrics['active_companies'] ?? 0 }}
                </div>
                <div class="text-sm text-slate-600 dark:text-slate-400">{{ __('home.companies_hiring') }}</div>
            </div>
        </div>
    </section>
    ...
    <section class="space-y-6">
        ...
        @else
            <div class="rounded-xl border border-slate-200/60 bg-white/60 p-12 text-center backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/40">
                <h3 class="text-lg font-semibold text-slate-900 dark:text-white">{{ __('home.no_roles_title') }}</h3>
