...
<a href="{{ localized_route('companies.show', ['slug' => $company->slug]) }}" class="group block rounded-xl border border-slate-200/60 bg-white/80 p-6 shadow-sm transition hover:shadow-md dark:border-slate-700/60 dark:bg-slate-900/60">
    <div class="flex items-start gap-4">
        ...
        <div class="flex-1 min-w-0">
            <h3 class="font-semibold text-lg text-slate-900 group-hover:text-indigo-600 dark:text-white dark:group-hover:text-indigo-400 transition">
                {{ $company->name }}
            </h3>
            @if($company->tagline)
                <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">{{ $company->tagline }}</p>
            @endif
