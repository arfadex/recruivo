...
<div class="space-y-8">
    ...
    @if($searchQuery || $remoteType || $location)
        <div class="space-y-4" x-data="{ showFilters: false }">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="inline-flex rounded-lg border border-slate-200 bg-white p-1 dark:border-slate-700 dark:bg-slate-800">
                    <a
                        ...
                        class="rounded-md px-4 py-2 text-sm font-medium transition {{ $filter === 'all' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white' }}"
                    >
                        {{ __('common.all') }}
                    </a>
                    <a
                        href="{{ localized_route('search', array_merge(request()->except('filter'), ['filter' => 'jobs'])) }}"
                        class="rounded-md px-4 py-2 text-sm font-medium transition {{ $filter === 'jobs' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white' }}"
                    >
                        {{ __('common.jobs') }}
                    </a>
                    <a
                        href="{{ localized_route('search', array_merge(request()->except('filter'), ['filter' => 'companies'])) }}"
                        class="rounded-md px-4 py-2 text-sm font-medium transition {{ $filter === 'companies' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white' }}"
                    >
                        {{ __('common.companies') }}
                    </a>
                </div>
                ...
                <button 
                    ...
                    class="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 sm:hidden"
                >
            ...
            {{-- Filter Panel --}}
            <div x-show="showFilters" x-cloak x-transition class="sm:!block rounded-xl border border-slate-200/60 bg-white/80 p-4 sm:p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
                <form method="GET" action="{{ localized_route('search') }}" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="space-y-2">
                        ...
                        <input
                            ...
                            class="w-full rounded-lg border border-slate-200/80 bg-white/80 px-4 py-2 text-sm text-slate-700 shadow-sm transition focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100 dark:focus:border-indigo-500"
                        />
                    </div>

                    {{-- Remote Type Filter --}}
                    <div class="space-y-2">
                        <label for="remote_type" class="block text-sm font-medium text-slate-700 dark:text-slate-300">
                            <div class="flex items-center gap-2">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <div class="flex items-end gap-2 sm:col-span-2 lg:col-span-2">
                        <button
                            type="submit"
                            class="flex-1 inline-flex items-center justify-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 transition hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200"
                        >
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <a
                            ...
                            class="flex-1 inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700"
                        >
                            <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
