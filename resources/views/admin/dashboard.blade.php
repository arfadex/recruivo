...
<div class="space-y-8">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white sm:text-3xl">{{ __('admin.dashboard') }}</h1>
            <p class="mt-2 text-sm text-slate-600 dark:text-slate-400 sm:text-base">{{ __('admin.dashboard_subtitle') }}</p>
        </div>
    ...
    <div class="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('admin.total_users') }}</p>
        ...
        <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('admin.total_jobs') }}</p>
        ...
        <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('admin.total_applications') }}</p>
        ...
        <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-slate-600 dark:text-slate-400">{{ __('admin.total_companies') }}</p>
                    <p class="text-2xl font-semibold text-slate-900 dark:text-white">{{ $totalCompanies }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid gap-6 md:grid-cols-2">
        <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">{{ __('admin.user_management') }}</h3>
            <div class="space-y-3">
                <a 
                    href="{{ localized_route('admin.users') }}" 
        ...
        <div class="rounded-xl border border-slate-200/60 bg-white/80 p-6 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
            <h3 class="text-lg font-semibold text-slate-900 dark:text-white mb-4">{{ __('admin.system_overview') }}</h3>
            <div class="space-y-3 text-sm text-slate-600 dark:text-slate-400">
                <div class="flex items-center justify-between">
                    <span>{{ __('admin.active_users') }}</span>
                    <span class="font-medium text-slate-900 dark:text-white">{{ $totalUsers }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span>{{ __('admin.published_jobs') }}</span>
                    <span class="font-medium text-slate-900 dark:text-white">{{ $totalJobs }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span>{{ __('admin.total_applications') }}</span>
                    <span class="font-medium text-slate-900 dark:text-white">{{ $totalApplications }}</span>
                </div>
                <div class="flex items-center justify-between">
                    <span>{{ __('admin.registered_companies') }}</span>
                    <span class="font-medium text-slate-900 dark:text-white">{{ $totalCompanies }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="rounded-xl border border-slate-200/60 bg-white/80 p-8 backdrop-blur dark:border-slate-700/60 dark:bg-slate-900/60">
        <h2 class="text-xl font-semibold text-slate-900 dark:text-white mb-6">{{ __('admin.system_status') }}</h2>
        <div class="grid gap-6 md:grid-cols-3">
            <div class="text-center">
                ...
                <h3 class="font-semibold text-slate-900 dark:text-white">{{ __('admin.system_online') }}</h3>
                <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('admin.all_services_running') }}</p>
            </div>
            <div class="text-center">
                ...
                <h3 class="font-semibold text-slate-900 dark:text-white">{{ __('admin.performance') }}</h3>
                <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('admin.optimal_response_times') }}</p>
            </div>
            <div class="text-center">
                <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-purple-100 dark:bg-purple-500/10">
                    <svg class="h-6 w-6 text-purple-600 dark:text-purple-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z" />
                    </svg>
                </div>
                <h3 class="font-semibold text-slate-900 dark:text-white">{{ __('admin.security') }}</h3>
                <p class="text-sm text-slate-600 dark:text-slate-400">{{ __('admin.all_systems_secure') }}</p>
            </div>
        </div>
    </div>
</div>
